...
export const getProductItem = async (productId) => {
  ...
  const [product, stock] = await Promise.all([
    ddbClient.send(new GetItemCommand(productParams)),
    ddbClient.send(new GetItemCommand(stockParams)),
  ...
  const productItem = product.Item && unmarshall(product.Item);
  const stockItem = stock && stock.Item ? unmarshall(stock.Item) : { count: 0 };
